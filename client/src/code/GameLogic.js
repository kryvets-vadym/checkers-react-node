    ...
    for (let i = 0; i < fakeBoard[0].length; i++) {
      for (let j = 0; j < fakeBoard[i].length; j++) {
        if (fakeBoard[i][j].player === playerNumber) {
          ...
          res = checkMove(fakeBoard, { i, j }, distance, isPlayer1, false, [
            { i: -1, j: -1 },
            {
              i: -1,
              j: 1,
...
const checkSingleMove = (fakeBoard, placeFrom, placeTo, isPlayer1, didEat) => {
  ...
  let fakeMove = fakeBoard.map((row, i) =>
    row.map((square, j) => Object.assign({}, square))
  )
  if (isValid) {
    if (eatNow) {
      ...
      eatingSizeDirection =
        1 +
        checkMove(fakeMove, placeTo, distance, isPlayer1, true, directions)
          .localMaxEating
    }
  }
    ...
    if (board[placeFrom.i][placeFrom.j].isKing) {
      ...
      if (
        ((isPlayer1 && placeTo.i === 0) ||
          (!isPlayer1 && placeTo.i === board.length - 1)) &&
        !board[placeFrom.i][placeFrom.j].isKing
      ) {
        isMoveToKing = true
      }
    }
